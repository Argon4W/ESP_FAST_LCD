...
esp_err_t esp_fast_text_engine_draw_native_outlined_string(
	...
	ESP_RETURN_ON_FALSE(text_engine_context		!= NULL, ESP_ERR_INVALID_ARG, ESP_FAST_TEXT_ENGINE_TAG, "No esp_fast_text_engine_instance_t handle provided when performing drawing an opaque native outlined string.");
	ESP_RETURN_ON_FALSE(panel_device_context	!= NULL, ESP_ERR_INVALID_ARG, ESP_FAST_TEXT_ENGINE_TAG, "No esp_fast_lcd_panel_device_t handle provided when performing drawing an opaque native outlined string.");
	...
	if (text_engine_context->properties->configuration.font_outline_radius == 0U) {
		return esp_fast_text_engine_draw_native_string(
	...
	const esp_fast_text_engine_instance_configuration_t*	configuration		= &	properties			->configuration;
	...
	const uint32_t											outline_radius		=	configuration		->font_outline_radius;
	const uint8_t											crlf_mode			=	configuration		->crlf_mode;
	...
	// Log the operation if LCD panel debug logging is enabled.
		...
		ESP_LOGD(ESP_FAST_TEXT_ENGINE_TAG, "Text engine instance \"%s\" is performing an opaque native outlined string draw at: positionX=%" PRId32 ", positionY=%" PRId32 ".",
	...
	while (*string != '\0') {
		...
		if (glyph_codepoint == 0x000AU) {
			glyph_advance_y += (int32_t) atlas_outline_size_y;
		...
		// Calculate the expanded glyph width.
		...
		), ESP_FAST_TEXT_ENGINE_TAG, "Failed to draw the outline ring glyph of the outlined string.");

		// Draw the regular glyph in the same way but with outline offsets.
		ESP_RETURN_ON_ERROR(esp_fast_lcd_draw_native_rectangle_masked(
			/* context			= */ panel_device_context,
			/* position_x		= */ position_x + (int32_t) outline_radius + glyph_advance_x,
			/* position_y		= */ position_y + (int32_t) outline_radius + glyph_advance_y,
			/* size_x			= */ glyph_size_x,
		...
		), ESP_FAST_TEXT_ENGINE_TAG, "Failed to draw the glyph of the outlined string.");

		// Advance the position X for the draw of next glyph.
		glyph_advance_x += (int32_t) outline_size_x;
	}
...
esp_err_t esp_fast_text_engine_draw_outlined_string(
	...
	// We cannot proceed without contexts.
	ESP_RETURN_ON_FALSE(text_engine_context		!= NULL, ESP_ERR_INVALID_ARG, ESP_FAST_TEXT_ENGINE_TAG, "No esp_fast_text_engine_instance_t handle provided when performing drawing a translucent outlined string.");
	ESP_RETURN_ON_FALSE(panel_device_context	!= NULL, ESP_ERR_INVALID_ARG, ESP_FAST_TEXT_ENGINE_TAG, "No esp_fast_lcd_panel_device_t handle provided when performing drawing a translucent outlined string.");

	// Route to regular native glyph draw if no outline in this text engine instance.
	if (text_engine_context->properties->configuration.font_outline_radius == 0U) {
		return esp_fast_text_engine_draw_string(
			/* text_engine_context	= */ text_engine_context,
			/* panel_device_context	= */ panel_device_context,
			/* position_x			= */ position_x,
			/* position_y			= */ position_y,
	...
	// Log the operation if LCD panel debug logging is enabled.
		...
		ESP_LOGD(ESP_FAST_TEXT_ENGINE_TAG, "Text engine instance \"%s\" is performing an translucent outlined string draw at: positionX=%" PRId32 ", positionY=%" PRId32 ".",
			/* s		*/ text_engine_context->properties->configuration.name,
			/* PRId32	*/ position_x,
			/* PRId32	*/ position_y
		);
