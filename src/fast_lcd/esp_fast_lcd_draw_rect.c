...
esp_err_t esp_fast_lcd_draw_native_rectangle(
	...
	ESP_RETURN_ON_FALSE(context != NULL, ESP_ERR_INVALID_ARG, ESP_FAST_LCD_TAG, "No esp_fast_lcd_panel_device_t handle provided when performing drawing an opaque native rectangle.");
	...
	// Calculate the count of ones in the dirty tile mask.
	...
	// Calculate the index of the first pixel of the first line at the framebuffer.
	const uint32_t dst_index =	/* index_y	= */ clipped_start_position_y * frame_size_x +
								/* index_x	= */ clipped_start_position_x;

	// Calculate the offset of the first pixel of the first line at the framebuffer.
	...
	// Log the operation if LCD panel debug logging is enabled.
		...
		ESP_LOGD(ESP_FAST_LCD_TAG, "LCD panel device \"%s\" is performing an opaque native rectangle draw at: positionX=%" PRId32 ", positionY=%" PRId32 ", sizeX=%" PRIu32 ", sizeY=%" PRIu32 ".",
	...
	if (clipped_size_x < 16U) {
		// Align the X to multiples of twos in order to fill the first line using u32.
		if (dst_index % 2U != 0U) {
			// Write the flipped color to framebuffer.
			dst_offset[x] = color_src_flipped;
			// Increment the cursor.
			x ++;
		}

		// Fill the first line of the rectangle with the u32 batch color.
		for (; x + 2U <= clipped_size_x; x += 2U) {
			// Fill the reinterpreted framebuffer with u32 combined colors.
			*((uint32_t*) (&dst_offset[x])) = color_src_flipped_32;
		}
		...
		asm volatile(vector_broadcast_16(q0, arg(0)) :: "a"(&color_src_flipped));

		// SIMD is 8 colors a batch (128-bit, 8 16-bit colors).
		for (uint16_t* simd_offset = &dst_offset[x]; x + 8U <= clipped_size_x; x += 8U) {
			// Store the broadcasted colors to the memory.
			// VST.128.IP will increment the %0 operand register automatically by the third operand (16).
			asm volatile(vector_store_128_aligned(q0, arg(0), 16) : "+a"(simd_offset));
		}
	...
	// Copy the data of the first line of the rectangle in the framebuffer to the rest of the lines.
	for (uint32_t y = 1U; y < clipped_size_y; y ++) {
		// Calculate the offset of the line (positionY + y) at the framebuffer.
		uint16_t* dst_offset2 = &dst_offset[y * frame_size_x];

		// Copy the data from the first line to the line (positionY + y).
		memcpy(
			/* dst_buffer	= */ dst_offset2,
...
esp_err_t esp_fast_lcd_draw_rectangle(
	...
	// We cannot proceed without context.
	ESP_RETURN_ON_FALSE(context != NULL, ESP_ERR_INVALID_ARG, ESP_FAST_LCD_TAG, "No esp_fast_lcd_panel_device_t handle provided when performing drawing a translucent rectangle.");
	...
	if (color_rgba8888_is_opaque(color_rgba8888)) {
		...
		return esp_fast_lcd_draw_native_rectangle(
			/* context			= */ context,
	...
	// Log the operation if LCD panel debug logging is enabled.
	#ifdef CONFIG_ESP_FAST_LCD_DEBUG_LOGGING
		ESP_LOGD(ESP_FAST_LCD_TAG, "LCD panel device \"%s\" is performing an translucent rectangle draw at: positionX=%" PRId32 ", positionY=%" PRId32 ", sizeX=%" PRIu32 ", sizeY=%" PRIu32 ".",
			/* s		*/ properties->configuration.name,
			/* PRId32	*/ position_x,
			/* PRId32	*/ position_y,
			/* PRIu32	*/ size_x,
	...
	if (clipped_size_x < 16) {
		...
		for		(uint32_t y = 0U; y < clipped_size_y; y ++) {
			for	(uint32_t x = 0U; x < clipped_size_x; x ++) {
				...
				// Get all color components of color_dst_rgb565.
				...
				// Map the destination RGB565 color to 0-255.
				...
				framebuffer[pixel_index] = color_dst_flipped;
			}
		}
	} else {
		...
		for (uint32_t y = 0U; y < clipped_size_y; y ++) {
			...
			for (; x < clipped_size_x && x < padding; x ++) {
				// Get the flipped original color of the pixel from the framebuffer.
				uint16_t color_flipped = dst_offset[x];

				// Flip the LSB and MSB to get the correct RGB565 color.
				const uint16_t color_dst_rgb565 =	((color_flipped >> 8U) & 0x00FFU)
				|									((color_flipped << 8U) & 0xFF00U);

				// Blend the dst color with pre-multiplied incoming colors.
				...
				// Flip the LSB and MSB back to get correct transmission byte order.
				color_flipped =	((color_final_rgb565 >> 8U) & 0x00FFU)
				|				((color_final_rgb565 << 8U) & 0xFF00U);

				dst_offset[x] = color_flipped;
			}

			// Broadcast the masks of the RGB565 format to the SIMD vector registers.
			asm volatile(
			...
			// SIMD is 8 colors a batch (128-bit, 8 16-bit colors).
			for (uint16_t* simd_offset = &dst_offset[x]; x + 8 <= clipped_size_x; x += 8) {
				...
				asm volatile(vector_load_128_aligned(q3, arg(0), 0) : "+a"(simd_offset));
				...
				// VST.128.IP will increment the %0 operand register automatically by the third operand (16).
				asm volatile(vector_store_128_aligned(q3, arg(0), 16) : "+a"(simd_offset));
			}
			...
			for (; x < clipped_size_x; x ++) {
				...
				// Blend the destination color with pre-multiplied incoming colors.
				const uint16_t color_final_rgb565 = private_blend_pre_mul_fast(
					/* r5_src_pre_mul	= */ r5_src_pre_mul,
					/* g6_src_pre_mul	= */ g6_src_pre_mul,
					/* b5_src_pre_mul	= */ b5_src_pre_mul,
